...
public class Exercise06_ElectricBill {
    ...
    public double calculateElectricBill(double unitsUsed, double unitsReturned) {
        ...
        if ((netUsage <= EXCESS_UNITS_LIMIT) && (unitsReturned > 0) && (netUsage > 0)) {
            return (netUsage*BASE_RATE) * DISCOUNT_FACTOR;
        } else if ((netUsage > EXCESS_UNITS_LIMIT) && (unitsReturned > 0)) {
            return (((EXCESS_UNITS_LIMIT*BASE_RATE) + ((netUsage-EXCESS_UNITS_LIMIT)*EXCESS_RATE))) * DISCOUNT_FACTOR;
        } else if (unitsReturned > 0 && netUsage <= 0) {
            return netUsage * BASE_RATE;
