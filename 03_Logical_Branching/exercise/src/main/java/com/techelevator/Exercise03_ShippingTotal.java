...
public class Exercise03_ShippingTotal {
    ...
    public double calculateShippingTotal(int weightPounds) {
        double chargePerPound = .50;
        double chargePerExtraPound = .75;
        if(weightPounds <= 40) {
            return weightPounds * chargePerPound;
        } else {
            return (40 * chargePerPound) + ((weightPounds - 40) * chargePerExtraPound);
        }
    ...
    public double calculateShippingTotal(int weightPounds, boolean hasDiscount) {
        double discount = 1.00 - 0.1;
        double chargePerPound = .50;
        double chargePerExtraPound = .75;

        if (weightPounds <= 40 && hasDiscount) {
            return weightPounds * chargePerPound * discount;
        } else if(weightPounds > 40 && hasDiscount) {
            return ((40 * chargePerPound) + ((weightPounds - 40) * chargePerExtraPound)) * discount;
    ...
    public double calculateShippingTotal(int weightPounds, double discountPercentage) {
        double chargePerPound = .50;
        double chargePerExtraPound = .75;
        double discount = 1.00 - discountPercentage;
        if(weightPounds <= 40) {
            return weightPounds * chargePerPound * discount;
        } else {
            return ((40 * chargePerPound) + ((weightPounds - 40) * chargePerExtraPound))* discount;
