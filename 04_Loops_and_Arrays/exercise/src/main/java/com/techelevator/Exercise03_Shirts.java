...
public class Exercise03_Shirts {
    ...
    public char[] buildOrder() {
        ...
        int numOfSmallShirts = 3;
        int numOfMedShirts = 2;
        int numOfLargeShirts = 1;
        for (int i = 0; i < order.length; i++) {
            if (i < numOfSmallShirts) {
                order[i] = SMALL_TSHIRT;
            } else if ((i >= numOfSmallShirts) && (i < (numOfSmallShirts + numOfMedShirts))) {
                order[i] = MEDIUM_TSHIRT;
            } else if (i >= (numOfSmallShirts + numOfLargeShirts)) {
    ...
    public char[] buildBulkOrder(int numberOfShirts) {
        ...
        for (int i = 1; i < bulkOrder.length; i += 3) {
            bulkOrder[i] = MEDIUM_TSHIRT;
        }
        for (int i = 2; i < bulkOrder.length; i += 3) {
    ...
    public boolean placeRequest(char[] order) {
        boolean orderMoreSmall = false;
        for (int i = 0; i < order.length; i++) {
            if (order[i] == SMALL_TSHIRT) {
                orderMoreSmall = true;
            }
        }
        return orderMoreSmall;
    }
}
